...
(defn split-obs
  "Split the observation into a vector of 2 observables.  Similar to split-with  
...
(defn await-observation 
  "Waits for the next observation from obs, returning the value received 
   during notification.  Identical to first-observation, except the promise is 
   immediately de-refed,  forcing the calling thread to block."
  [origin]
  @(first-observation origin)) 

(defn await-n-observations 
  "Waits for the next observation from obs, returning the value received during 
   notification.  Identical to take-observations, except the promise is 
   immediately de-refed, forcing the calling thread to block."
  [n origin]
