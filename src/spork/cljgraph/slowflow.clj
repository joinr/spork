...
;;       (let [^long f (.flow info)]
;;         (if (> f 0)
;;           (assoc! acc (clojure.lang.MapEntry. (.from info) (.to info))  f) acc))) 
;;     (transient {}) 
;;     (get-edge-infos! g))))
  ...
  (loop [xs  (get-edge-infos! g)
    ...
    (if-let [^einfo info (first xs)]
      (let [^long f (.flow info)]
        (recur (rest xs)
               (if (> f 0)
                 (cons acc (clojure.lang.MapEntry. (clojure.lang.MapEntry. (.from info) (.to info))  f))
                 acc)))
      acc)))
    ...
    (if (nil? (next xs)) acc
        (let [from (first  xs)
          ...
          (recur (conj acc (if (forward? g from to)
                             (.assoc ^einfo (edge-info g from to) :dir :increment)
                             (.assoc ^einfo (edge-info g to from) :dir :decrement)))
                 (next xs))))))
