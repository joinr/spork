(ns spork.cljgraph.visual.vijual
                             ...
                             (when (> width 1)
                               (let [index (- ypos y 1)
                                     s (if (>= index (count text))
                                         ...
                                         (text index))]
                                 (str s 
                                    (fill \space (- width (count s) 2)) \|)))))
                    [overlapping s] (let [[x2 width2] 
                                          (loop [k more]
                                            (when-let 
                                              [[{x2 :x 
                                                 width2 :width 
...
(defn label-text 
  "Returns a text string representing the label for the item. It handles the 
   special case of keywords, so that :oak-tree ==> 'oak tree'"
  [text]
     (if (keyword? text)
...
(defn wrap [text width]
  "This function optimally wraps text to fit within a given number of characters
   , given a monospace font"
  (let [text (letfn [[f [text]
                      (lazy-seq (if (<= (count text) width)
                                  [text]
                                  (let [spc (positions (partial = \space) 
                                                       (take width text))]
                                    (if (seq spc)
                                      (cons (take (last spc) text) 
                                            (f (drop (inc (last spc)) text)))
                                      (cons (take width text) 
                                            (f (drop width text)))))))]]
...
;Scan Functions- A "scan" is a run-length-encoded list of heights that are used 
;to optimally calculate packing of tree and graphs. An example scan would be 
;[[0 5] [7 10]] which would mean "The height is 5 for an x between 0 
;(inclusive) and 7 (exclusive). The height is 10 for any x greater than or 
;equal to 7."
...
(defn scan-lowest-y
  "Finds the lowest y that is available at x with width y that would prevent it 
   from intersecting with the scan."
  [scan x width]
  (loop [scan scan
         cury nil
         besty nil]
...
;During ascii rendering, text in boxes will wrap after this many characters.
...
(def ascii-dim {:width-fn (fn [text]
                 ...
                 :wrap-fn (fn 
                            ([text]
                               (vec (map #(apply str " " %) 
                                         (wrap text ascii-wrap-threshold))))
                            ([text width height]
                               (vec (map #(apply str " " %) 
                                         (center (wrap text (- width 4)) 
                                                 (- width 4) (- height 3))))))
                 :node-padding 1
                 :row-padding 8
                 :line-wid 1
...
(defn tree-to-shapes 
  "Converts a full layed-out tree into a bunch of shapes to be sent to backend 
   rendering code."
  [{:keys [wrap-fn line-wid]} tree]
  (mapcat (fn [{:keys [text x y width height 
                       line-left line-right line-ypos leaf parent-line-y]}]
            (concat (when parent-line-y
                      [{:type :rect :x (+ x (half width) 
                                          (- (half line-wid))) 
                        :y parent-line-y 
                        :width line-wid :height (inc (- y parent-line-y))}])
                    [{:type :rect :text (wrap-fn text width height) :x x :y y 
                      :width width :height height}]
                    (when-not leaf
                      [{:type :rect :x line-left :y line-ypos 
                        :width (- line-right line-left) :height line-wid}
                       {:type :rect :x (+ x (half width) (- (half line-wid))) 
                        :y (dec (+ y height)) :width line-wid 
                        :height (+ (- line-ypos y height) 2)}])))
          tree))

(defn make-rows 
  "Takes a tree and converts it into rows. This is needed since items in the 
   same row and their widths will affect the spacing and layout of items."
  [tree]
  (when (seq tree)
...
(defn wrap-text
  "calculates the wrapped text of each tree item and the resulting height 
   based on how the text was broken into lines"
...
(defn row-pos 
  "Calculates preliminary x positions for nodes in a tree. This will be refined 
   later by the calculations from the 'space' functions."
...
(defn space-row
  "This function calculates the x positions of tree nodes. All calculations 
   start from the longest row in the tree. This function is then used to 
   position nodes upwards or downwards from the longest row. Each row is 
   positioned relative a 'target row', which is the neighboring row nearest 
   to the longest row."
           ...
           (let [children (filter #(fun % item) target-row)
                 ...
                 nu-x (if (seq child-pos)
                        (let [child (first children)
                              ...
                              left-scoot (if (and (< 1 (count siblings)) 
                                                  (= ((first siblings) :id) id))
                                           (half (- 
                                                   (apply + 
                                                     (map 
                                                       #(+ (% :width) 
                                                   ...
                                                   (child :width)))
                                           0)
                              k (- (half (+ (first child-pos) (last child-pos)))
                                   (half width) left-scoot)
...
(defn horz-lines [{:keys [line-wid]} rows]
  "This function calculates the left and right extents of the horizontal line 
   below a node that leads to its children."
  (map (fn [cur next]
         (map (fn [{:keys [id] :as cur}]
                (let [bounds (fn [{:keys [x width]}]
                               (+ x (half width)))
                      ranges (cons (bounds cur)
                                   (for [{:keys [parent] :as chi} next 
                                         :when (= id parent)]
                                     (bounds chi)))]
                  (merge cur {:line-left (- (apply min ranges) 
                                            (half line-wid))
                              :line-right (+ (apply max ranges) 
                                             (half line-wid))})))
              cur))
       rows
       (concat (rest rows) [[]])))
...
(defn level-lines 
  "This function is used to stagger horizontal lines vertically in an optimal 
   fashion, so that nodes can connect to their children in a tree with the least
   amount of inbetween space."
         ...
         (let [group-right (atom 0)
           ...
           (map (fn [{:keys [leaf line-left line-right x width] :as item}]
                  (cond leaf 
                        item
                        (and @group-right 
                             (> (+ @group-right line-wid) line-left))
                        (swap! line-y
                               (if (<= (+ x (half width)) 
                                       (+ @group-right line-padding))
                                 dec
                                 inc))
                        true
...
(defn place-boxes [{:keys [line-padding] :as dim} scan acc row]
  ...
  (if-let [[{:keys [x width height] :as item} & r] (seq row)]
    (let [y (scan-lowest-y scan x width)]
      (recur dim (scan-add scan x (+ y height line-padding) width) 
             (cons (assoc item :y y) acc) r))
    [(reverse acc) scan]))
  ...
  (if (seq row)
    (let [[{:keys [line-left line-right leaf] :as item} & r] row
      ...
      (if leaf
        ...
        (recur dim (scan-add scan line-left 
                     (+ cury line-wid line-padding) line-width) 
               (cons (assoc item :line-ypos cury) acc) r)))
    [(reverse acc) scan]))

(defn pack-tree 
  "gets rid of extra empty space in a tree by moving up nodes and horizontal 
...
(defn layout-tree [{:keys [row-padding height width width-fn] :as dim} tree]
  ...
  (let [rows (make-rows tree)
        ...
        pos (map vector (map 
                          (partial row-pos dim) wrapped (iterate inc 0)) widths)
...
(defn graph-to-shapes
  "Converts a graph into a list of shapes which is used by backend ascii or 
   bitmap pipelines to render to a medium."
  [{:keys [line-padding line-wid wrap-fn]} tree]
  (mapcat (fn [{:keys [text xpos ypos width height links]}]
            (cons {:type :rect :text (wrap-fn text width height) :x xpos :y ypos 
                   :width width :height height}
                  (mapcat (fn [{:keys [legs dest arrow]}]
                           (let [shapes (map (fn [{x1 :xpos y1 :ypos dir1 :dir} 
                                                  {x2 :xpos y2 :ypos dir2 :dir}]
                                                (if (or (horizontal dir1) 
                                                        (vertical dir2))
                                                  {:type :rect :x (min x1 x2) 
                                                   :y (min y1 y2) 
                                                   :width (+ (abs (- x2 x1)) 
                                                             line-wid) 
                                                   :height line-wid}
                              ...
                              (concat [{:type (if (= arrow :start) 
                                      ...
                                      (-> (vec shapes)
                                          ...
                                          (assoc-in [(dec (count shapes)) :on-top] 
                                                    true)))))
                          links)))
       tree))
...
(defn get-side
  "All graphs currently start as logically square. This function therefore 
   calculates length of side with the sqrt funciton."
  [col]
  (int (ceil (Math/sqrt (count col)))))

(defn shuffle-nodes
  "Randomly swaps two nodes of the graph.
   Tom Spoon: this will bone up on small graphs, where 
   probability of a and b being the same increases.
   Changed to ensure b is not a."
  [pos nodes]
  (let [a (rand-nth nodes)
        b (rand-nth (filter #(not (#{a} %)) nodes))                           
        an (pos a)
        bn (pos b)]
    (merge pos 
...
(defn anneal 
  "A naive annealing algorithm for initial layout of nodes in graphs. The 
   current code is crude and very prone to getting stuck in local maxima. 
   Best replaced with a genetic algorithm in the future."
  [edges nodemap]
  (let [nodes (distinct (concat (apply concat edges) (keys nodemap)))
        ...
        pos (into {}
                  (map (fn [s e]
                         [s {:x (mod e side) :y (int (/ e side)) 
                             :text (label-text (or (nodemap s) s))}])
                       (shuffle nodes)
                       (range nodenum)))]
    (loop [pos pos
...
(defn tension-directed
  "Calculates the total length of all edges, based on manhattan distance. 
   Directional arrows pointing upwards are penalized, so that the graph will 
   have mainly downward arrows."
  [edges pos]
  (apply + 
         (map (fn [[f t]]
                (let [{fx :x fy :y} (pos f)
...
(defn depth-sort
  "Sorts nodes based on their depth (i.e. does it have children, grandchildren, 
   etc)"
  [nodes edges]
...
(defn anneal-directed
  "This function anneals a directed graph. The nodes are started off in a 
   depth-sorted way, with the hopes that this will make it more likely that 
   arrows will be predominantly pointing downwards."
  [edges nodemap]
...
(defn fill-details
  "This function fills in the width & height & text wrapping for each node, 
   calculated from the appropriate functions."
  [{:keys [width-fn height-fn wrap-fn]} nodes]
  (into {}
        (map (fn [[k {:keys [text] :as item}]]
               [k (assoc item :width (width-fn text) :height (height-fn text) 
                         :wrapped-text (wrap-fn text))])
             nodes)))

(defn add-lines
  "This function connects nodes by lines based on their edges. Depending on the 
   relative locations of the nodes, the lines will 'snake around' other nodes in 
   different ways."
  [nodes edges]
  (into {}
...
(defn vline-groups
  "Returns a list of all vertical lines, grouped by what columns of nodes they 
   fall between."
  [nodes]
...
(defn hline-groups
  "Returns a list of all horizontal lines, grouped by what rows of nodes they 
   fall between."
  [nodes]
...
(defn pos-map
  "Builds a map keyed on node location. (Used early in the code when position is
   still determined by row/column slots, not by exact x/y coordinates."
  [nodes]
  (into {}
        (map (fn [[key {:keys [x y]}]]
               [[x y] key])
...
(defn reverse-links
  "Searches the graph for all edges/links and creates a map keyed on the 
...
(defn loose-layout
  "This is the first-pass function for laying out a graph. It makes the graph 
   extremely sparse, which is fixed later on by the vcompact function."
  [{:keys [line-padding line-wid height-fn width-fn]} nodes]
  (let [side (get-side nodes)
        row-heights 
        (vec (map (fn [y] (apply max (cons 0 (map #(do (height-fn (% :text))) 
                                                 (filter #(= (% :y) y) 
                                                         (vals nodes))))))
                              (range side)))
        col-widths 
        (vec (map (fn [x] (apply max (cons 0 (map #(width-fn (% :text)) 
                                                  (filter #(= (% :x) x) 
                                                          (vals nodes))))))
                             (range side)))
        row-tots 
        (vec (cons 0 (reductions + (map #(+ % line-padding) row-heights))))
        col-tots 
        (vec (cons 0 (reductions + (map #(+ % line-padding) col-widths))))
        ...
        hlines-map (into {}
                         (for [[n group] (numbered hlines)
                           ...
                           [line m]))
        vline-counts 
        (vec (cons 0 (reductions + (map #(* (count %) (+ line-padding line-wid)) 
                                        vlines))))
        hline-counts 
        (vec (cons 0 (reductions + (map #(* (count %) (+ line-padding line-wid)) 
                                        hlines))))]
    (into {}
          (map (fn [[key {:keys [x y links text] :as item}]]
                 (let [xpos (+ (col-tots x) (vline-counts (inc x)) 
                               (half (- (col-widths x) (width-fn text))))
                       ypos (+ (row-tots y) (hline-counts (inc y)) 
                               (half (- (row-heights y) (height-fn text))))]
                   [key
                    (assoc item 
                      :xpos xpos
                      :ypos ypos
                                                 ...
                                                 (assoc pt1 
                                                   :xpos (if (ratio? xx)
                                                           ...
                                                           (+ (col-tots xx) 
                                                              (vline-counts xx) 
                                                              (* 
                                                                (vlines-map [key link-index (+ index -1 xsrc)]) (+ line-padding line-wid))))
                                                   :ypos (if (ratio? yy)
                                                           (if (< index 2)
                                                             ypos
                                                             ypos2)
...
(defn vcompact
  "This function is a monster and will probably be broken into smaller functions
   in the future. Its job is to pack the graph by pushing all nodes and lines 
   upwards as much as possible. It also 'feathers out' all edges attaching to 
   the same side of a single node, so that you can see separate attachment 
   points for each."
...
(defn mirror
  "Flips the graph along a diagonal axis going from the upper-left to 
   bottom-right corner."
  [nodes]
  (into {}
...
(defn hline-map
  "This builds a map, keyed on the y position, of all the lines in a graph. 
   This lets us efficiently check for collisions."
  [nodes]
...
(defn line-unblocked
  "Checks a linemap (created by hline-map, probably) to see if a new line will 
   cause a collision."
  [linemap y x1 x2]
  (> 2 
...
(defn remove-zigzags 
  "Nodes that are adjacent are connected by a 'zigzag' line for most of the 
   algorithms, so that they have some 'give'. This function removes any zigzags 
   for cases where nodes are close enough to be connected by just a straight 
   line."
  [{:keys [line-padding line-wid]} nodes]
...
(defn mark-directed
  "All edges (aka links) are directed from the node closest to the upper-left
   corner to the other node. This is independent in directed graphs as to which 
   way the arrow is pointing. This function checks the 'direction' of the edge 
   and marks whether the arrow head goes on the start or end of the edge."
  [nodes edges]
...
(defn layout-graph
  "Takes edges and nodes and generates a list of nodes with detailed attributes 
...
(defn line-info-btree 
  "Takes 2 rows of nodes in the binary tree and figures out the arrangement of
   lines between the two rows."
  [top bottom]
...
(defn layout-btree 
  "Takes a binary tree and converts it into rows, with nodes at the same level 
   of the tree in the same row. The data for each item in each row is a vector, 
   formatted as [indentation width text left-children? right-children?"
  [btree]
  (if btree
...
(defn render-btree [rows]
  "Renders a binary tree. The rows handed to it contain the indentation already, 
   so this function is concerned solely with rendering this row data to ASCII"
  (let [x rows]
    (loop [rows x]
      (when (seq rows)
        (let [row (first rows)]
...
(defn draw-graph
  "Draws an undirected graph to the console. Requires a list of pairs 
   representing the edges of the graph. Additionally, a separate map can be 
   included containing node information mapped via the node ids in the edge 
   list."
  ([edges nodes]
...
(defn draw-directed-graph
  "Draws an undirected graph to the console. Requires a list of pairs 
   representing the edges of the graph. Additionally, a separate map can be 
   included containing node information mapped via the node ids in the edge 
   list. An effort is made to have majority of 'arrows' in the graph move in a 
   downward direction."
  ([edges nodes]
...
(defn draw-binary-tree
  "Draws a binary tree to the console. Nodes are in the form [text left right]
   where 'left' and 'right' are optional fields containing the children of this 
   node."
  [btree]
...
(defn draw-graph-image
  "Draws an undirected graph to a java image. Requires a list of pairs 
   representing the edges of the graph. Additionally, a separate map can be 
   included containing node information mapped via the node ids in the edge 
   list."
  ([edges nodes]
...
(defn draw-directed-graph-image
  "Draws an directed graph to a java image. Requires a list of pairs 
   representing the edges of the graph. Additionally, a separate map can be 
   included containing node information mapped via the node ids in the 
   edge list."
  ([edges nodes]
