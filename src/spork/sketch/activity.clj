;;sketch-compatible primitives for rendering temporal data,
;;event-based "activities" as tracks on a scrollable display.
;;We'll build on this later, notably by porting it to
;;piccolo, so that we have a nice multitrack display for
;;visualizing event histories.  This lib was originally
;;built to support publication for a WinterSim paper on
;;stochastic sampling using DSLs, but it has potential
;;application elsewhere.
;;See spork.examples.activityvis for the full example.
;;Note: this was ported from a different implementation
;;of sketch, so there are currently some visual artifacts
;;that need to be resolved (like cartesian coordinate basis).
;;It's close enough, but should be migrated to something
;;that renders accurately under the current library.
(ns spork.sketch.activity
  (:require [spork.sketch :refer :all]
            ;[spork.sketch.legacy :refer [cartesian]]
            [spork.graphics2d.canvas :as c :refer [shape-bounds]]
            [spork.geometry.shapes :refer [->rectangle]]))

;;Note:
;;For compatibility, we turn the cartesian operation (was a
;;shape combinator) into a no-op, since the rendering backend
;;handles cartesian systems for us by default.  not working
;;entirely correctly right now, particularly with text and
;;axis labels.
(defmacro cartesian [body]
  `~body)
...
(defmacro with-event->color [event->color & body]
  `(binding [~'spork.sketch.activity/*event->color*  ~event->color]
     ~@body))

(def ^:dynamic *track-options* {:activity-labels true 
                                :track-labels true})
(defmacro hiding-labels [& body]
  `(binding [~'spork.sketch.activity/*track-options* (merge ~'spork.sketch/*track-options* 
                                                   {:activity-labels nil 
                                                    :track-labels nil})]
     ~@body))

(defmacro with-track-scale [expr & body]
  `(binding [~'spork.sketch.activity/*track-options* (assoc ~'spork.sketch/*track-options* 
                                                   {:track-scale ~expr})]
     ~@body))
