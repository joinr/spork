...
(defn bbox
  "Defines a bounding box, rooted at (x,y), width wide, height high,
   ...
   performed."
  ([x y width  height theta] (->boundingbox x y (max (Math/abs (double width)) 1.0) (max (Math/abs (double height)) 1.0) theta))
  ([x y width  height] (->boundingbox x y (max (Math/abs (double width)) 1.0) (max (Math/abs (double height)) 1.0) 0)))
